...
readline.question(header("Number : "), num => {
    ...
    if (i >= 0 && i < plugins.length) {
        ...
        const meta = require(process.cwd() + "/" + PLUGIN_DIR + plugins[i] + "/plugin.js");
        if (meta.attributes.name && meta.attributes.version && meta.attributes.description && meta.attributes.category) {
            console.log(success(meta.attributes.name + " v" + meta.attributes.version));
