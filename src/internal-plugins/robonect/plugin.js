...
function loaded(api) {
    ...
    class Robonect {
         ...
         */
        refresh() {
            this.getStatus((err, res) => {
                if (!err) {
                    this.registerTile(res);
                } else {
        ...
        registerTile(res) {
            ...
            if (res.status.status != null && res.status.status != "undefined") {
                // 0=DETECTING_STATUS
            ...
            if (res.status.battery != null && res.status.battery != "undefined") {
            ...
            if (res.status.mode != null && res.status.mode != "undefined") {
                let title = "mower.na";
                let icon = "";
                ...
                if (res.blades != null && res.blades != "undefined") {
                    miniTiles.push({icon: iconBlades, text: res.blades.quality + "%", colorDefault: api.themeAPI.constants().DARK_COLOR_KEY});
                }
            ...
            const tile = api.dashboardAPI.Tile("robonect-all", api.dashboardAPI.TileType().TILE_SUB_TILES, null, null, null, null, null, null, 0, 107, null, miniTiles, api.webAPI.Authentication().AUTH_USAGE_LEVEL);
